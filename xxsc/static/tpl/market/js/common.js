...
function addToCart(goodsId, parentId, isnowbuy, extCode) {
	...
	if (isSelectAttr(spec_arr)) {
		...
		var tid = "";
		if (goods.spec.length == 1)
		{
		    tid = goods.spec[0];
		} else {
		    tid = goods.spec[0]+","+goods.spec[1];
		}


		$.post(domain_url + "u/usercenter/addcart", { pId: goods.goods_id, tId: tid, q: goods.number }, function (d) {
            if (d.Result==0)
                alert(d.Msg);
            if (d.Result == 1)
                window.location.href = domain_url + "u/usercenter/cart";
            if (d.Result ==3)
                window.location.href = domain_url + "mk/login";
