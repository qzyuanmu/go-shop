...
var ChkPhoneUrl = 'http://'+document.domain+'/Login/ChkPhone';
...
function checkEmailExist(email, callback) {
	...
	$.post('http://www.tcw3721.com/js/register.php?act=check_email_exist', {
		email: email
	}, function(result) {
		if (result == 'false') {
...
function sendEmailCode(emailObj, emailCodeObj, sendButton) {
	checkEmail(emailObj, function(result) {
		if (result) {
			...
			var url = 'http://www.tcw3721.com/js/register.php?act=send_email_code';
			$.post(url, {
				email: emailObj.val()
			}, function(result) {
				if (result == 'ok') {
...
function sendMobileCode(mobileObj, mobileCodeObj, sendButton) {
    ...
    checkMobilePhone(mobileObj, function (result) {
		if (result) {
			// 发送邮件
			var url = 'http://www.tcw3721.com/js/register.php?act=send_mobile_code';
			$.post(url, {
				XDEBUG_SESSION_START: 'ECLIPSE_DBGP',
